...
function App() {
    ...
    // popup lifetime
    useEffect(() => {
        if (!checkPopupId) return
        const timer = window.setTimeout(() => setCheckPopupId(null), 500)
        return () => window.clearTimeout(timer)
    }, [checkPopupId])

    useEffect(() => {
        if (!pendingPopupVisible) return
        const timer = window.setTimeout(() => setPendingPopupVisible(false), 500)
        return () => window.clearTimeout(timer)
    }, [pendingPopupVisible])
    ...
    const pairValid = useMemo(() => {
        if (!baseCode || !quoteCode) return false
        if (isCodesLoading) return false
        return baseCode !== quoteCode
    }, [baseCode, quoteCode, isCodesLoading])

    const disableActions = !pairValid
    ...
    const handleCheckUpdate = async (row: UpdateRow) => {
        ...
        try {
            const [latest] = await Promise.all([fetchRateUpdate(row.updateId), wait(250)])
            setUpdates((prev) =>
                prev.map((item) =>
    ...
    return (
        <main className="app">
            ...
            <form className="controls" onSubmit={handleScheduleUpdate}>
                    ...
                    <div className="pair-picker">
                        <label>
                            ...
                            <select
                                ...
                                value={baseCode}
                                onChange={handleSelectChange(setBaseCode)}
                                disabled={isCodesLoading || !codes.length}
                            >
                                {codes.map((code) => (
                        ...
                        <button
                            ...
                            disabled={isCodesLoading || !codes.length}
                        >
                            <ArrowLeftRight size={28} strokeWidth={2.25} aria-hidden="true" />
                        </button>
                        ...
                        <label>
                            ...
                            <select
                                ...
                                value={quoteCode}
                                onChange={handleSelectChange(setQuoteCode)}
                                disabled={isCodesLoading || !codes.length}
                            >
                                {codes.map((code) => (
                                    <option value={code} key={code}>{code}</option>
                ...
                {!pairValid && !isCodesLoading && (
                    <span className="inline-warning" role="status" aria-live="polite">
                        Choose different base and quote.
                    </span>
                )}
            </form>

            {codesError && <p className="error" role="alert">Currencies: {codesError}</p>}
            {actionError && <p className="error" role="alert">{actionError}</p>}

            <section className="latest-rate">
                <header>
                    <h2>Latest rate</h2>
                    {!latestRate && <span className="hint" role="status" aria-live="polite">Choose a pair and click &quot;Get latest rate&quot;</span>}
                </header>

                {isRateLoading && !latestRate && (
                    <dl className="rate-skeleton" role="status" aria-live="polite">
                        <div>
                            <dt>Pair</dt>
                            <dd><span className="skeleton-box" style={{ width: '92px' }} /></dd>
                        </div>
                        <div>
                            <dt>Value</dt>
                            <dd><span className="skeleton-box" style={{ width: '64px' }} /></dd>
                        </div>
                        <div>
                            <dt>Updated</dt>
                            <dd><span className="skeleton-box" style={{ width: '140px' }} /></dd>
                        </div>
                    </dl>
                )}
                ...
                {latestRate && (
                    <dl>
                        <div>
                            <dt>Pair</dt>
                            <dd>{latestRate.base}/{latestRate.quote}</dd>
                        </div>
                        <div>
                            <dt>Value</dt>
                                        ...
                                        <div className="value-main">
                                            {row.status === 'pending' ? (
                                                <button
                                                    ...
                                                    aria-busy={row.checking ? 'true' : 'false'}
                                                >
                                                    <ArrowPathIcon className="value-icon" />
                                                </button>
                                            ) : (
